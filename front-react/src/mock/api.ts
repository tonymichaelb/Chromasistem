...
function defaultBrushMixtures() {
  ...
  return {
    ...
    tintaColors: {} as Record<string, string>,
  };
}
...
export function getMockResponse(
  ...
  if (path === "/api/me")
    return jsonResponse({ success: true, username: "dev" });
  if (path === "/api/logout" && method === "POST") return jsonResponse({});

  // Printer — status padrão: conectada e ociosa para o passo a passo funcionar sem inserir dados (MOCK=true)
  if (path === "/api/printer/status")
    return jsonResponse({ success: true, status: defaultPrinterStatus });
  if (path === "/api/printer/connect" && method === "POST")
    return jsonResponse({ success: true });
  if (path === "/api/printer/disconnect" && method === "POST")
    return jsonResponse({ success: true });
  if (path === "/api/printer/start" && method === "POST")
    return jsonResponse({ success: true });
  if (path === "/api/printer/pause" && method === "POST")
    return jsonResponse({ success: true });
  if (path === "/api/printer/resume" && method === "POST")
    return jsonResponse({ success: true });
  if (path === "/api/printer/stop" && method === "POST")
    return jsonResponse({ success: true });
  // Detecção de falhas e skip (Task 4)
  if (path === "/api/printer/failure" && method === "POST")
    return jsonResponse({
      success: true,
      message: "Falha registrada; aguardando ação",
    });
  if (path === "/api/printer/failure/resolve" && method === "POST")
    return jsonResponse({
      success: true,
      message: "Aguardando problema resolvido",
    });
  if (path === "/api/printer/failure/resolved" && method === "POST")
    return jsonResponse({
      success: true,
      message: "Problema resolvido; retomando impressão",
    });
  if (path === "/api/printer/skip-object" && method === "POST")
    return jsonResponse({
      success: true,
      message: "Pulando item; impressão continuará no próximo objeto",
    });
      ...
      entries: [
        {
          ...
          failure_message: "Filamento travado.",
          action: "skipped",
          object_index_or_name: "0",
        },
        {
          id: 2,
          print_job_id: 1,
          ...
          failure_code: null,
          failure_message: "Falha detectada na impressão",
          action: "detected",
          object_index_or_name: null,
        },
      ],
    });
  }
  ...
  if (path === "/api/printer/current-brush")
    return jsonResponse({ success: true, brush: 0 });
  if (path === "/api/printer/load-brush-mixtures")
    return jsonResponse({ success: true, ...defaultBrushMixtures() });
  if (path === "/api/printer/select-brush" && method === "POST")
    return jsonResponse({ success: true });
  if (path === "/api/printer/send-mixture" && method === "POST")
    return jsonResponse({ success: true });
  if (path === "/api/printer/save-brush-mixtures" && method === "POST")
    return jsonResponse({ success: true });
  if (path === "/api/printer/gcode" && method === "POST")
    return jsonResponse({ success: true, response: "ok" });
  if (path === "/api/printer/commands-history")
    return jsonResponse({ success: true, history: [], count: 0 });

  // Files
  if (path === "/api/files/list")
    return jsonResponse({ success: true, files: defaultFilesList });
  ...
  if (printMatch && method === "POST")
    return jsonResponse({ success: true, message: "Impressão iniciada" });
